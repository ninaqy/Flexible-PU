...
def _variable_with_weight_decay(name, shape, stddev, wd, use_xavier=True, zero_init=False):
...
def batch_norm_dist_template(inputs, is_training, scope, moments_dims, bn_decay):
  ...
  with tf.variable_scope(scope) as sc:
    ...
    normed = tf.cond(tf.cast(is_training, tf.bool),
                     train_bn_op,
                     test_bn_op)
    return normed
...
def pairwise_distance2(pc1, pc2):
  """Compute pairwise distance of a point cloud.
  ...
  Returns:
    pairwise distance:(b,n,m)
  """
  N = pc1.get_shape().as_list()[1]
  M = pc2.get_shape().as_list()[1]

  pc2_transpose = tf.transpose(pc2, perm=[0, 2, 1])
  pc_inner = tf.matmul(pc1, pc2_transpose)
  pc_inner = -2*pc_inner
  pc1_square = tf.reduce_sum(tf.square(pc1), axis=-1, keep_dims=True)
  pc2_square = tf.reduce_sum(tf.square(pc2), axis=-1, keep_dims=True)
  pc2_square_tranpose = tf.transpose(pc2_square, perm=[0, 2, 1])
  return pc1_square + pc_inner + pc2_square_tranpose
...
def knn_value(adj_matrix, k=20):
  """Get KNN based on the pairwise distance.
  Args:
    pairwise distance: (batch_size, num _points, num_points)
    k: int
  ...
  """
  neg_adj = -adj_matrix
  nn_value, nn_idx = tf.nn.top_k(neg_adj, k=k)
  return nn_idx, nn_value
