...
def feature_extraction_sin(inputs, scope='feature_extraction2', is_training=True, bn_decay=None):
    ...
    with tf.variable_scope(scope,reuse=tf.AUTO_REUSE):
        ...
        for l in range(L):
            for i in range(3):
                pc_single = inputs[:,:,i]
                pc_sin = tf.math.sin(2**l *np.pi* pc_single)
                pc_cos = tf.math.cos(2**l *np.pi* pc_single)
                pc_freq = tf.concat([pc_freq, tf.expand_dims(pc_sin, -1), tf.expand_dims(pc_cos,-1)], axis=-1)
...
def up_projection_unit2(inputs,up_ratio,scope="up_projection_unit",is_training=True,bn_decay=None):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        L = conv2d(inputs, 128, [1, 1],
                               padding='VALID', stride=[1, 1],
                               bn=False, is_training=is_training,
...
def up_projection_unit3(inputs,up_ratio,scope="up_projection_unit",is_training=True,bn_decay=None):
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        L = conv2d(inputs, 128, [1, 1],
                               padding='VALID', stride=[1, 1],
                               bn=False, is_training=is_training,
